...
function renderEmptyState(title, message) {
  return `
    <div class="empty-state">
      ...
      <p class="meta">${escHtml(message)}</p>
    </div>
  `;
}
...
function renderMadeAttempt(made, attempt) {
  if (made === null || made === undefined || attempt === null || attempt === undefined) {
    return "";
  }
  return `${escHtml(made)}/${escHtml(attempt)}`;
}

function renderPlayerTrendsPanel(container, detail) {
  const panel = container.querySelector("[data-player-trends-panel]");
  ...
  if (trends.length === 0) {
    panel.innerHTML = renderEmptyState(
      "Trend table is unavailable.",
      "Recent versus prior windows are not available for this player yet."
    );
    return;
  }
  const rows = trends
    .map((item) => {
      const deltaClass =
        item.direction_is_good === true
          ? "good"
          : item.direction_is_good === false
            ? "bad"
            : "";
      const pctChange =
        item.pct_change === null || item.pct_change === undefined
          ? ""
          : `${escHtml(item.pct_change)}%`;
      return `
        <tr>
          <td>${escHtml(item.label || item.stat || "")}</td>
          <td>${renderValue(item.recent_avg)}</td>
          <td>${renderValue(item.prior_avg)}</td>
          <td class="${deltaClass}">${renderValue(item.delta)}</td>
          <td>${pctChange}</td>
        </tr>
      `;
    })
...
function setupPlayerTrendCharts() {
  document.querySelectorAll("[data-player-trend-chart]").forEach((chart) => {
    ...
    if (chart.dataset.playerTrendLazy === "true") {
      return;
    }
...
function hydrateLazyTrendChart(container, detail) {
  const chart = container.querySelector('[data-player-trend-chart][data-player-trend-lazy="true"]');
  if (!(chart instanceof HTMLElement)) {
    return;
  }
  const games = Array.isArray(detail.game_log?.games) ? detail.game_log.games : [];
  const baselines = isObject(detail.chart_baselines) ? detail.chart_baselines : {};
  const group = chart.closest("[data-tab-group]") || container;
  const controls = group.querySelector("[data-player-trend-controls]");
  const render = (statKey) => {
    chart.innerHTML = buildTrendSvg(games, statKey, baselines);
  };
  if (controls instanceof HTMLElement && controls.dataset.playerTrendHydrated !== "true") {
    controls.dataset.playerTrendHydrated = "true";
    controls.querySelectorAll("[data-stat-key]").forEach((button) => {
      button.addEventListener("click", () => {
        controls.querySelectorAll("[data-stat-key]").forEach((item) => {
          item.classList.toggle("is-active", item === button);
  ...
  const active = controls instanceof HTMLElement
    ? controls.querySelector(".is-active[data-stat-key]")
    : null;
  render(active?.dataset.statKey || "pts");
}

function renderPlayerGameLogPanel(container, detail) {
  const panel = container.querySelector("[data-player-game-log-panel]");
  if (!(panel instanceof HTMLElement)) {
    return;
  }
  const games = Array.isArray(detail.game_log?.games) ? [...detail.game_log.games].reverse() : [];
  if (games.length === 0) {
    panel.innerHTML = renderEmptyState(
      "Game log is unavailable.",
      "No game-level rows are available for this qualified player yet."
    );
    return;
  }
  const rows = games
    .map((game) => {
      const matchup = game.matchup || game.opponent_abbr || "";
      const result = game.wl ? `<span class="meta">${escHtml(game.wl)}</span>` : "";
      return `
        <tr>
          <td>${renderValue(game.game_date)}</td>
          <td>${escHtml(matchup)} ${result}</td>
          <td>${renderValue(game.min)}</td>
          <td>${renderValue(game.pts)}</td>
          <td>${renderValue(game.reb)}</td>
          <td>${renderValue(game.ast)}</td>
          <td>${renderValue(game.stl)}</td>
          <td>${renderValue(game.blk)}</td>
          <td>${renderValue(game.tov)}</td>
          <td>${renderValue(game.fg3m)}</td>
          <td>${renderMadeAttempt(game.fgm, game.fga)}</td>
          <td>${renderMadeAttempt(game.ftm, game.fta)}</td>
          <td>${renderValue(game.fantasy_points_simple)}</td>
        </tr>
      `;
    })
    .join("");
  panel.innerHTML = `
    <div class="table-scroll">
      <table class="data-table game-log-table">
        <thead>
          <tr>
            <th>Date</th>
            <th>Matchup</th>
...
function renderSimilarPlayersPanel(container, detail) {
  const panel = container.querySelector("[data-similar-players-panel]");
  if (!(panel instanceof HTMLElement)) {
    return;
  }
  const state = detail.panel_states?.similarity;
  const items = Array.isArray(detail.similar_players) ? detail.similar_players : [];
  if (state !== "fresh" || items.length === 0) {
    panel.innerHTML = renderEmptyState(
      "Similar-player matches are unavailable.",
      detail.similarity_reason || "The similarity model does not have a stable profile for this player yet."
    );
    return;
  }
  const anchorId = detail.player?.player_id || container.dataset.playerId;
  const cards = items
    .map((item) => {
      const sharedTraits = Array.isArray(item.shared_traits) ? item.shared_traits : [];
      const contrastingTraits = Array.isArray(item.contrasting_traits) ? item.contrasting_traits : [];
      const sharedMarkup = sharedTraits
        .map((trait) => `<span class="chip">Shared ${escHtml(trait)}</span>`)
        .join("");
      const contrastMarkup = contrastingTraits
        .map((trait) => `<span class="chip">Diff ${escHtml(trait)}</span>`)
        .join("");
      return `
        <article class="tracked-card">
          <div class="card-header">
            <div class="identity-row">
              ${renderHeadshot(item)}
              <div>
                <h3><a href="/players/${escHtml(item.player_id)}">${escHtml(item.player_name) || "Unknown player"}</a></h3>
                <p class="meta">${escHtml(item.team_abbr) || "NBA"} · ${escHtml(item.archetype_label) || "Archetype pending"}</p>
              </div>
            </div>
            <span class="status">Similarity ${renderValue(item.similarity_score)}</span>
          </div>
          <div class="chip-row">
            ${sharedMarkup}
            ${contrastMarkup}
            <a class="button-link secondary" href="/compare?player_a_id=${escHtml(anchorId)}&player_b_id=${escHtml(item.player_id)}">Compare</a>
          </div>
        </article>
      `;
    })
    .join("");
  panel.innerHTML = `<div class="stack-list">${cards}</div>`;
}

function renderPlayerHydrationError(container) {
  const message = "Could not load this panel. Refresh the page to try again.";
  const trendPanel = container.querySelector("[data-player-trends-panel]");
  const chart = container.querySelector("[data-player-trend-chart]");
  const gameLogPanel = container.querySelector("[data-player-game-log-panel]");
  const similarPanel = container.querySelector("[data-similar-players-panel]");
  if (trendPanel instanceof HTMLElement) {
    trendPanel.innerHTML = renderEmptyState("Trend table failed to load.", message);
  }
  if (chart instanceof HTMLElement) {
    chart.innerHTML = renderEmptyState("Trend chart failed to load.", message);
  }
  if (gameLogPanel instanceof HTMLElement) {
    gameLogPanel.innerHTML = renderEmptyState("Game log failed to load.", message);
  }
  if (similarPanel instanceof HTMLElement) {
    similarPanel.innerHTML = renderEmptyState("Similar players failed to load.", message);
  }
}

function hydratePlayerDetailPanels() {
  document.querySelectorAll("[data-player-detail-hydrate]").forEach(async (container) => {
    if (!(container instanceof HTMLElement)) {
      return;
    }
    if (container.dataset.playerDetailHydrated === "true") {
      return;
    }
    ...
    container.dataset.playerDetailHydrated = "true";
    try {
      const response = await fetch(`/api/players/${encodeURIComponent(playerId)}`, { cache: "default" });
      const payload = await response.json();
      if (!response.ok) {
        throw new Error(payload.detail || "player detail unavailable");
      }
      const detail = payload.item || {};
      hydrateLazyTrendChart(container, detail);
      renderPlayerTrendsPanel(container, detail);
      renderPlayerGameLogPanel(container, detail);
      renderSimilarPlayersPanel(container, detail);
    } catch {
      renderPlayerHydrationError(container);
    }
  });
}
